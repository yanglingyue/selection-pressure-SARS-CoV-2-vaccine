...
outpath <-  "02_model-output/all_vaccine_jama"
dir.create(outpath)
...
model1 <-  gam(Y1 ~ npi_cb+FBW_jama_cb+N+travel_cb+s(C, bs = "re")+P:V,method = "REML",data=df)
summary(model1)
AIC(model1)
...
cp_FW <- crosspred(FBW_jama_cb, model1, cen=0,by=0.1,at=0:A/10,bylag=0.1,cumul=TRUE) #slices
...
S_O <- read.csv(paste0(outpath,"/vero_pressure_selection_slice_2022_S.CSV"))
...
S1_O <- read.csv(paste0(outpath,"/vero_pressure_selection_slice_2022_S1.CSV"))
...
S2_O <- read.csv(paste0(outpath,"/vero_pressure_selection_slice_2022_S2.CSV"))
...
ggsave(p,filename = file.path("03_fig-main","figS8-lower-VE-jama.pdf"),
       width = 16,height =6)
