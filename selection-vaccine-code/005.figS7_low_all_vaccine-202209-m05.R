...
outpath <-  "02_model-output/all_vaccine_202201_202209-m05"
dir.create(outpath)
...
png(file=paste0(outpath,"/fig_vero_slice_",gene_type[g],".png"),width=820,height=800,res=110)

col1 <- "#00468BFF"
  tcol1 <- do.call(rgb, c(as.list(col2rgb(col1)), alpha = 255/3, max = 255))
  par(mfrow = c(1, 1), oma = c(0.5, 0.5, 3, 0.5))
  plot(cp_FW,"slices",lag=3,cumul=T,
       col=col1, ci.arg=list(col = tcol1),mgp=c(3,1,0),
       cex.axis=1.5,cex.lab=1.5,cex.main=1.5,lwd=3,family = "ArialN",
       ci.level=0.95,col=2,xlab="Adjusted vaccine coverage",
       ylab="Effect on selection pressure")
  #mtext("Lag3",side = 3,line =1,cex=1.5,family = "ArialN",font=2)
  mtext(paste0(gene_type[g]), side = 3, font=2,family = "ArialN",cex=1.8 ,line = 0, outer = T)
...
ggsave(p,filename = file.path("03_fig-main"
                              ,"figS7-202003-202110-m05.pdf"),
       width = 16,height =6)
