...
dir.create(outpath0)
outpath <-  paste0(outpath0,"/","fitness_vaccine_202003_202110")
dir.create(outpath)
outpath1 <-  paste0(outpath,"/","vero_result",sep="")
dir.create(outpath1)
outpath2 <-  paste0(outpath0,"/","fitness_NI_202003_202110")
dir.create(outpath2)
...
# Extract estimates for mutational fitness model when the adjusted vaccine coverage is 25%
...
# Extract estimates for mutational fitness model when the adjusted vaccine coverage is 50%
...
# Extract estimates for mutational fitness model when the adjusted vaccine coverage is 75%
...
write.csv(result_vero,file=paste0(outpath1,"/vero_fitness_error_bar_",gene_type[g],".csv"),row.names =F)
...
write.csv(result_vero_all,file=paste0(outpath,"/vero_fitness_slice_",gene_type[g],".csv"),row.names =F)
...
#Model the association between natural immunity coverage and mutational fitness
...
for(g in 14)
{
  ...
  # Extract estimates for mutational fitness model when the natural immunity is 25%
  ...
  # Extract estimates for mutational fitness model when the natural immunity is 50%
  ...
  # Extract estimates for mutational fitness model when the natural immunity is 75%
  ...
  write.csv(result_NI,file=paste0(outpath3,"/NI_fitness_error_bar_",gene_type[g],".csv"),row.names =F)
  
  # Extract the all estimates for the association between mutational fitness and natural immunity
  ...
  write.csv(result_NI_all,file=paste0(outpath2,"/NI_fitness_slice_",gene_type[g],".csv"),row.names =F)
