...
setwd(root_path)
dat1<-read.csv("01_data/subsample_adaptation_vaccine_high.csv")
...
p1  <- ggplot(D1_vero,aes(x=gene, y=vero_fit,color=coverage,group=coverage))+ 
  ...
  scale_y_continuous(limits = c(-12,4),breaks = seq(-12,4,4))+
...
ggsave(p,filename = file.path(root_path,"/",outpath0,"/","figure_S14.pdf"),
