...
dir.create("04_extended_analysis/plot_output/Figure_S3")
outpath <-  "04_extended_analysis/plot_output/Figure_S3/"
...
#col1 <-  colorRampPalette(brewer.pal(11,"PiYG")[6:10])
#col2 <- colorRampPalette(brewer.pal(10,"PiYG")[2:6])
col1 <- colorRampPalette(c("#5c4591",'#a786b9',"#d4b9da","#e7e1ef","#F7F7F7"))
col2 <- colorRampPalette(brewer.pal(11,"RdBu")[6:10])
...
ps1 <- ggplot(dat1_S1, aes(country,month)) +
  geom_tile(aes(fill=dnds_mean_focal),color = "white",size=0.3) +
  #geom_text(aes(label = values), color = "black",size=3) + 
  scale_fill_gradientn(colors = cols, limits = c(0, 9), na.value = "grey80",
                       breaks = seq(0, 9, by = 1)) + 
...
ps2 <- ggplot(dat1_S2, aes(country,month)) +
  geom_tile(aes(fill=dnds_mean_focal),color = "white",size=0.3) +
  #geom_text(aes(label = values), color = "black",size=3) + 
  scale_fill_gradientn(colors = cols, limits = c(0, 9), na.value = "grey80",
                       breaks = seq(0, 9, by = 1)) + 
...
ggsave(patchwork,filename = file.path(paste0(outpath,"fig_S3.pdf")),
       width = 20,height =18)
...
ps1 <- ggplot(dat1_S1, aes(country,month)) +
  geom_tile(aes(fill=muts_nonsyn_focal),color = "white",size=0.3) +
  #geom_text(aes(label = values), color = "black",size=3) + 
  scale_fill_gradientn(colors = cols, limits = c(0, 9), na.value = "grey80",
                       breaks = seq(0, 9, by = 1)) + 
  scale_y_discrete(breaks=c("2020-03","2020-09","2021-03",
  ...
  theme(legend.position = "right",    legend.key.size=unit(2,'cm'),
        legend.key.width = unit(3,"cm"),legend.key.height  = unit(0.5,"cm"))+
  guides(fill = guide_colorbar("Nonsynonymous to synonymous divergence ratio", title.position = "right",
                               title.theme = element_text(size = 20, angle = 90, hjust = 0.5),
                               label.theme = element_text(size = 20),
                               title.vjust = 0.9,ticks.linewidth = 0.5,lty='solid',  # Set tick line width
                               ticks.colour = "black")) +
...
ps2 <- ggplot(dat1_S2, aes(country,month)) +
  ...
  theme(legend.position = "bottom",    legend.key.size=unit(2,'cm'),
        legend.key.width = unit(4,"cm"),legend.key.height  = unit(0.5,"cm"))+
  guides(fill = guide_colorbar("Nonsynonymous to synonymous divergence ratio", title.position = "top",
                               title.theme = element_text(size = 20, angle = 0, hjust = 0.5),
                               label.theme = element_text(size = 20),
                               title.vjust = 0.9,ticks.linewidth = 0.5,lty='solid',  # Set tick line width
                               ticks.colour = "black")) +
