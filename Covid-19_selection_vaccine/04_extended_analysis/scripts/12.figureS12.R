...
dir.create(outpath0)
outpath1 <-  paste0(outpath0,"/","dnds_vaccine_202201_202209")
dir.create(outpath1)
outpath2 <-  paste0(outpath0,"/","nonsyn_vaccine_202201_202209")
dir.create(outpath2)
outpath3 <-  paste0(outpath0,"/","fitness_vaccine_202201_202209")
dir.create(outpath3)
...
V <- data$FW_BW_P_jama # for adjusted vaccine coverage (considering the lower vaccine efficacy for the Omicron variant compared to other VOCs)
...
model3 <-  gam(Y1 ~ npi_cb+FBW_jama_cb+N+travel_cb+s(C, bs = "re")+P:V,method = "REML",data=df)
summary(model3)
AIC(model3)
...
cp_FW <- crosspred(FBW_jama_cb, model3, cen=0,by=0.1,at=0:A/10,bylag=0.1,cumul=TRUE) #slices
...
write.csv(result,file=paste0(outpath1,"/","vero_dnds_slice_2022_",gene_type[g],".csv",sep=""),row.names =F)
...
for(g in 14)
{
  ...
  result <- as.data.frame(result)
  result$gene <- gene_type[g]
  write.csv(result,file=paste0(outpath3,"/","vero_fitness_slice_2022_",gene_type[g],".csv",sep=""),row.names =F)
...
S_O <- read.csv(paste0(root_path,"/",outpath1,"/","vero_dnds_slice_2022_S1.CSV"))
...
DF1 <- merge(S,S_O,by=c("vero_coverage"),all.x = T)
...
DF2 <- merge(S1,S1_O,by=c("vero_coverage"),all.x = T)
...
S2_O <- read.csv(paste0(root_path,"/",outpath3,"/","vero_fitness_slice_2022_S1.CSV"))
colnames(S2_O) <- c("S2O_dat_vero_low","S2O_dat_vero_fit","S2O_dat_vero_high","vero_coverage" ,"S2O")


DF3 <- merge(S2,S2_O,by=c("vero_coverage"),all.x = T)
...
ggsave(p,filename = file.path(outpath0,"figure_S12.pdf"),
