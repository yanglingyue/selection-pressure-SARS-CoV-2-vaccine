...
dir.create("04_extended_analysis/plot_output/Figure_S4")
outpath <-  "04_extended_analysis/plot_output/Figure_S4/"
  ...
  
{
  ...
  G3=G2[,c("country","month","gene","FW_BW_P","F_Vero","travel_route","NPI_NV","new_cases_month","dnds_mean_focal")]
  
  var <- c("month","country","dnds_mean_focal")
  alldata <- G3
...
dat3= subset(dat2,dat2$gene== "S"|dat2$gene== "E"|dat2$gene== "M"|dat2$gene== "N"|dat2$gene== "ORF1a"|
               dat2$gene== "ORF1b"|dat2$gene== "ORF3a"|dat2$gene== "ORF6"|dat2$gene== "ORF7a"|
               dat2$gene== "ORF7b"|dat2$gene== "ORF8"|dat2$gene== "ORF9b")

dat3$gene <-factor(dat3$gene,levels= c('S','E','M','N','ORF1a','ORF1b','ORF3a','ORF6',
...
p1 <-ggplot(dat3,aes(x=month,y=dnds_mean))+
  ...
  annotate(geom = "text", x = "2021-01", y =2.05,angle=90, label = "Gamma", hjust = 0,size =4.8)+
  annotate(geom = "text", x = "2021-05", y =2.05,angle=90, label = "Delta", hjust =0,size = 4.8)+
  annotate(geom = "text", x = "2020-12", y =2.05,angle=90, label = "Alpha/Beta", hjust = 0,size = 4.8)+
  annotate(geom = "text", x = "2021-11", y =2.05,angle=90, label = "Omicron", hjust =0,size = 4.8)+
  ...
  annotate(geom = "text", x = "2021-06", y =3.9,angle=0, label = "Vaccine rollout", hjust =0,size = 5.5)+
...
ggsave(p1,filename =file.path(paste0(outpath,"fig_S4.pdf")),
