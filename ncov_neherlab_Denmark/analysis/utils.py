    ...
    for node in tree.find_clades():
        ...
        for parent in parents:
                ...
                if "S" in parent.branch_attrs["mutations"]:
                    ...
                    for mut in parent.branch_attrs["mutations"]["S"]:
                        ...
                        if int(mut[1:-1]) in range(14,686):
                            parents_s1_muts+=[mut]
                            if int(mut[1:-1]) in range(319,541):
                                parents_rbd_muts+=[mut]
            ...
            if hasattr(parent, 'node_attrs') and 'syn_muts' in parent.node_attrs:
                ...
                if 'RBD' in parent.node_attrs['syn_muts']:
                    parents_rbd_syn += parent.node_attrs['syn_muts']['RBD']    
                ...
                if 'ORF3a' in parent.node_attrs['syn_muts']:
                    parents_orf3a_syn += parent.node_attrs['syn_muts']['ORF3a'] 
                if 'ORF6' in parent.node_attrs['syn_muts']:
                    parents_orf6_syn += parent.node_attrs['syn_muts']['ORF6']
                if 'ORF7a' in parent.node_attrs['syn_muts']:
                    parents_orf7a_syn += parent.node_attrs['syn_muts']['ORF7a']
                if 'ORF7b' in parent.node_attrs['syn_muts']:
                    parents_orf7b_syn += parent.node_attrs['syn_muts']['ORF7b']
                if 'ORF8' in parent.node_attrs['syn_muts']:
                    parents_orf8_syn += parent.node_attrs['syn_muts']['ORF8']
                if 'ORF9b' in parent.node_attrs['syn_muts']:
                    parents_orf9b_syn += parent.node_attrs['syn_muts']['ORF9b'] 
                if 'ORF1a' in parent.node_attrs['syn_muts']:
                    parents_orf1a_syn += parent.node_attrs['syn_muts']['ORF1a']
                if 'ORF1b' in parent.node_attrs['syn_muts']:
                    parents_orf1b_syn += parent.node_attrs['syn_muts']['ORF1b']     
        ...
        parents_rbd_muts = remove_reversions(parents_rbd_muts)
        ...
        parents_rbd_syn = remove_reversions(parents_rbd_syn)
        ...
        parents_orf1a_syn = remove_reversions(parents_orf1a_syn)
        parents_orf1b_syn = remove_reversions(parents_orf1b_syn)
        parents_orf3a_syn = remove_reversions(parents_orf3a_syn)
        parents_orf6_syn = remove_reversions(parents_orf6_syn)
        parents_orf7a_syn = remove_reversions(parents_orf7a_syn)
        parents_orf7b_syn = remove_reversions(parents_orf7b_syn)
        parents_orf8_syn = remove_reversions(parents_orf8_syn)
        parents_orf9b_syn = remove_reversions(parents_orf9b_syn)
        ...
        rbd_mutation_count = consolidate_deletions(parents_rbd_muts)
        ...
        node.node_attrs["rbd_syn_accumulation"] = len(parents_rbd_syn)
        node.node_attrs["s2_syn_accumulation"] = len(parents_s2_syn)
        node.node_attrs["spike_syn_accumulation"] = len(parents_spike_syn)
        node.node_attrs["n_syn_accumulation"] = len(parents_n_syn)
        node.node_attrs["e_syn_accumulation"] = len(parents_e_syn)
        node.node_attrs["m_syn_accumulation"] = len(parents_m_syn)
        node.node_attrs["orf1a_syn_accumulation"] = len(parents_orf1a_syn)
        node.node_attrs["orf1b_syn_accumulation"] = len(parents_orf1b_syn)
        node.node_attrs["orf3a_syn_accumulation"] = len(parents_orf3a_syn)
        node.node_attrs["orf6_syn_accumulation"] = len(parents_orf6_syn)
        node.node_attrs["orf7a_syn_accumulation"] = len(parents_orf7a_syn)
        node.node_attrs["orf7b_syn_accumulation"] = len(parents_orf7b_syn)
        node.node_attrs["orf8_syn_accumulation"] = len(parents_orf8_syn)
        node.node_attrs["orf9b_syn_accumulation"] = len(parents_orf9b_syn)
        ...
        node.node_attrs["rbd_accumulation"] = rbd_mutation_count
        ...
        node.node_attrs["orf1a_accumulation"] = orf1a_mutation_count
        node.node_attrs["orf1b_accumulation"] = orf1b_mutation_count
...
def add_del_accumulation_attr(tree):
    ...
    for node in tree.find_clades():
            ...
            if hasattr(parent, "branch_attrs") and "mutations" in parent.branch_attrs:
                ...
                if "ORF7a" in parent.branch_attrs["mutations"]:
                    for mut in parent.branch_attrs["mutations"]["ORF7a"]:
                        if mut[-1] == '-':
                            parents_orf7a_dels+=parent.branch_attrs["mutations"]["ORF7a"]
                # find ORF9b muts
                if "ORF9b" in parent.branch_attrs["mutations"]:
                    for mut in parent.branch_attrs["mutations"]["ORF9b"]:
                        if mut[-1] == '-':
                            parents_orf9b_dels+=parent.branch_attrs["mutations"]["ORF9b"]
                            
                 # find ORF8 muts
                if "ORF8" in parent.branch_attrs["mutations"]:
                    for mut in parent.branch_attrs["mutations"]["ORF8"]:
                        if mut[-1] == '-':
                            parents_orf8_dels+=parent.branch_attrs["mutations"]["ORF8"]
               
                 # find ORF7b muts
                if "ORF7b" in parent.branch_attrs["mutations"]:
                    for mut in parent.branch_attrs["mutations"]["ORF7b"]:
                        if mut[-1] == '-':
                            parents_orf7b_dels+=parent.branch_attrs["mutations"]["ORF7b"]
                            
                 # find ORF6 muts
                if "ORF6" in parent.branch_attrs["mutations"]:
                    for mut in parent.branch_attrs["mutations"]["ORF6"]:
                        if mut[-1] == '-':
                            parents_orf6_dels+=parent.branch_attrs["mutations"]["ORF6"]
                            
                 # find ORF9b muts
                if "ORF3a" in parent.branch_attrs["mutations"]:
                    for mut in parent.branch_attrs["mutations"]["ORF3a"]:
                        if mut[-1] == '-':
                            parents_orf3a_dels+=parent.branch_attrs["mutations"]["ORF3a"]
                 
                 # find ORF1a muts
                if "ORF1a" in parent.branch_attrs["mutations"]:
                    for mut in parent.branch_attrs["mutations"]["ORF1a"]:
                        if mut[-1] == '-':
                            parents_orf1a_dels+=parent.branch_attrs["mutations"]["ORF1a"]
                            
                 # find ORF1b muts
                if "ORF1b" in parent.branch_attrs["mutations"]:
                    for mut in parent.branch_attrs["mutations"]["ORF1b"]:
                        if mut[-1] == '-':
                            parents_orf1b_dels+=parent.branch_attrs["mutations"]["ORF1b"]
        ...
        rbd_deletion_count = consolidate_deletions(parents_rbd_dels)
        ...
        orf9b_deletion_count = consolidate_deletions(parents_orf9b_dels)
        orf8_deletion_count = consolidate_deletions(parents_orf8_dels)
        orf7b_deletion_count = consolidate_deletions(parents_orf7b_dels) 
        orf6_deletion_count = consolidate_deletions(parents_orf6_dels)
        orf3a_deletion_count = consolidate_deletions(parents_orf3a_dels)
        orf1a_deletion_count = consolidate_deletions(parents_orf1a_dels)
        orf1b_deletion_count = consolidate_deletions(parents_orf1b_dels)
        
        # add node attribute that lists the number of deletions accumulated at each node
        node.node_attrs['deletion_accumulation']= {'S': spike_deletion_count,
                                                   'S1': s1_deletion_count, 'S2':s2_deletion_count,
                                                   'RdRp':rdrp_deletion_count, 'Nsp6':nsp6_deletion_count,
                                                   'RBD':rbd_deletion_count,
                                                   'Nsp4':nsp4_deletion_count, 'N':n_deletion_count,
                                                   'M': m_deletion_count, 'E': e_deletion_count,
                                                   'ORF7a': orf7a_deletion_count,
                                                   'ORF9b': orf9b_deletion_count, 'ORF8': orf8_deletion_count,
                                                   'ORF7b': orf7b_deletion_count, 'ORF6': orf6_deletion_count,
                                                   'ORF3a': orf3a_deletion_count,'ORF1a': orf1a_deletion_count,
                                                   'ORF1b': orf1b_deletion_count}
        # add node attribute that lists the number of nonsynonymous SNPs accumulated at each node
        node.node_attrs['nonsyn_snps_accumulation'] = {'S': node.node_attrs["spike_accumulation"] - spike_deletion_count,
                                                       'S1': node.node_attrs["s1_accumulation"] - s1_deletion_count,
                                                       'S2': node.node_attrs["s2_accumulation"] - s2_deletion_count,
                                                       'RBD': node.node_attrs["rbd_accumulation"] - rbd_deletion_count,
                                                       ...
                                                       'ORF1b': node.node_attrs["orf1b_accumulation"] - orf1b_deletion_count,
                                                       'ORF1a': node.node_attrs["orf1a_accumulation"] - orf1a_deletion_count,
                                                       'ORF7a': node.node_attrs["orf7a_accumulation"] - orf7a_deletion_count,
                                                       'ORF9b': node.node_attrs["orf9b_accumulation"] - orf9b_deletion_count,
                                                       'ORF8': node.node_attrs["orf8_accumulation"] - orf8_deletion_count,
                                                       'ORF7b': node.node_attrs["orf7b_accumulation"] - orf7b_deletion_count,
                                                       'ORF6': node.node_attrs["orf6_accumulation"] - orf6_deletion_count,
                                                       'ORF3a': node.node_attrs["orf3a_accumulation"] - orf3a_deletion_count}
...
def add_mut_at_node_attr(tree):
    ...
    for node in tree.find_clades():
        ...
        rbd_syn_at_this_node = []
        spike_syn_at_this_node = []
        ...
        orf9b_syn_at_this_node = []
        orf8_syn_at_this_node = []
        orf7b_syn_at_this_node = []
        orf6_syn_at_this_node = []
        orf3a_syn_at_this_node = []
        orf1a_syn_at_this_node = []
        orf1b_syn_at_this_node = []
        ...
        if hasattr(node, "node_attrs"):
            ...
            if 'RBD' in node.node_attrs['syn_muts']:
                rbd_syn_at_this_node.append(node.node_attrs['syn_muts']['RBD']) 
            if 'S' in node.node_attrs['syn_muts']:
                spike_syn_at_this_node.append(node.node_attrs['syn_muts']['S'])     
            ...
            if 'ORF9b' in node.node_attrs['syn_muts']:
                orf9b_syn_at_this_node.append(node.node_attrs['syn_muts']['ORF9b']) 
            if 'ORF8' in node.node_attrs['syn_muts']:
                orf8_syn_at_this_node.append(node.node_attrs['syn_muts']['ORF8'])
            if 'ORF7b' in node.node_attrs['syn_muts']:
                orf7b_syn_at_this_node.append(node.node_attrs['syn_muts']['ORF7b'])
            if 'ORF6' in node.node_attrs['syn_muts']:
                orf6_syn_at_this_node.append(node.node_attrs['syn_muts']['ORF6'])
            if 'ORF3a' in node.node_attrs['syn_muts']:
                orf3a_syn_at_this_node.append(node.node_attrs['syn_muts']['ORF3a'])
            if 'ORF1a' in node.node_attrs['syn_muts']:
                orf1a_syn_at_this_node.append(node.node_attrs['syn_muts']['ORF1a'])
            if 'ORF1b' in node.node_attrs['syn_muts']:
                orf1b_syn_at_this_node.append(node.node_attrs['syn_muts']['ORF1b'])   
        ...
        node.syn_at_node['RBD'] = len(rbd_syn_at_this_node)
        node.syn_at_node['S'] = len(spike_syn_at_this_node)
        ...
        node.syn_at_node['ORF9b'] = len(orf9b_syn_at_this_node)
        node.syn_at_node['ORF8'] = len(orf8_syn_at_this_node)
        node.syn_at_node['ORF7b'] = len(orf7b_syn_at_this_node)
        node.syn_at_node['ORF6'] = len(orf6_syn_at_this_node)
        node.syn_at_node['ORF3a'] = len(orf3a_syn_at_this_node)
        node.syn_at_node['ORF1a'] = len(orf1a_syn_at_this_node)
        node.syn_at_node['ORF1b'] = len(orf1b_syn_at_this_node)
        
        if hasattr(node, 'branch_attrs'):

            s1_nonsyn_at_this_node = []
            s2_nonsyn_at_this_node = []
            rbd_nonsyn_at_this_node = []
            ...
            if "S" in node.branch_attrs["mutations"]:
                for mut in node.branch_attrs["mutations"]["S"]:
                    if int(mut[1:-1]) in range(14,686):
                        ...
                        if int(mut[1:-1]) in range(319,541):
                            rbd_nonsyn_at_this_node.append(mut)
                    elif int(mut[1:-1]) in range(687,1274):
            ...
            rbd_consolidated = consolidate_deletions_2(rbd_nonsyn_at_this_node)
            node.nonsyn_at_node['RBD'] = len(rbd_consolidated)
            
            spike_nonsyn_at_this_node = []
            if "S" in node.branch_attrs["mutations"]:
                for mut in node.branch_attrs["mutations"]["S"]:
                    spike_nonsyn_at_this_node.append(mut)

            spike_consolidated = consolidate_deletions_2(spike_nonsyn_at_this_node)
            node.nonsyn_at_node['S'] = len(siple_consolidated)
            ...
            orf1a_nonsyn_at_this_node = []
            if "ORF1a" in node.branch_attrs["mutations"]:
                for mut in node.branch_attrs["mutations"]["ORF1a"]:
                    orf1a_nonsyn_at_this_node.append(mut)

            orf1a_consolidated = consolidate_deletions_2(orf1a_nonsyn_at_this_node)
            node.nonsyn_at_node['ORF1a'] = len(orf1a_consolidated)
            
            orf1b_nonsyn_at_this_node = []
            if "ORF1b" in node.branch_attrs["mutations"]:
                for mut in node.branch_attrs["mutations"]["ORF1b"]:
                    orf1b_nonsyn_at_this_node.append(mut)

            orf1b_consolidated = consolidate_deletions_2(orf1b_nonsyn_at_this_node)
            node.nonsyn_at_node['ORF1b'] = len(orf1b_consolidated)
...
def add_changes_from_root_attr(tree):
    ...
    for node in tree.find_clades():
        ...
        for parent in parents:
            if hasattr(parent, "branch_attrs") and "mutations" in parent.branch_attrs:
                ...
                if "ORF1b" in parent.branch_attrs["mutations"]:
                    parents_orf1b_muts+=parent.branch_attrs["mutations"]["ORF1b"]
                # find RBD muts
                if "RBD" in parent.branch_attrs["mutations"]:
                    parents_rbd_muts+=parent.branch_attrs["mutations"]["RBD"]   
        ...
        rbd_mutation_list = consolidate_deletions_2(parents_rbd_muts)
        ...
        orf9b_mutation_list = consolidate_deletions_2(parents_orf9b_muts)
        orf8_mutation_list = consolidate_deletions_2(parents_orf8_muts)
        orf7b_mutation_list = consolidate_deletions_2(parents_orf7b_muts)
        orf6_mutation_list = consolidate_deletions_2(parents_orf6_muts)
        orf3a_mutation_list = consolidate_deletions_2(parents_orf3a_muts)
        orf1a_mutation_list = consolidate_deletions_2(parents_orf1a_muts)
        orf1b_mutation_list = consolidate_deletions_2(parents_orf1b_muts)
        ...
        rbd_syn_mutation_list = consolidate_deletions_2(parents_rbd_syn)
        ...
        orf9b_syn_mutation_list = consolidate_deletions_2(parents_orf9b_syn)
        orf8_syn_mutation_list = consolidate_deletions_2(parents_orf8_syn)
        orf7b_syn_mutation_list = consolidate_deletions_2(parents_orf7b_syn)
        orf6_syn_mutation_list = consolidate_deletions_2(parents_orf6_syn)
        orf3a_syn_mutation_list = consolidate_deletions_2(parents_orf3a_syn)
        orf1a_syn_mutation_list = consolidate_deletions_2(parents_orf1a_syn)
        orf1b_syn_mutation_list = consolidate_deletions_2(parents_orf1b_syn)
        
        node.node_attrs["changes_from_ref"] = {'s1_non':s1_mutation_list, 's2_non':s2_mutation_list, 'rdrp_non':rdrp_mutation_list, 'spike_non':spike_mutation_list, 'e_non':e_mutation_list, 'n_non':n_mutation_list, 'm_non':m_mutation_list, 'nsp6_non':nsp6_mutation_list, 'orf7a_non':orf7a_mutation_list,'orf9b_non':orf9b_mutation_list,'orf8_non':orf8_mutation_list,'orf7b_non':orf7b_mutation_list,'orf6_non':orf6_mutation_list,'orf3a_non':orf3a_mutation_list,'orf1a_non':orf1a_mutation_list,'orf1b_non':orf1b_mutation_list,'rbd_non':rbd_mutation_list,'s1_syn': s1_syn_mutation_list, 's2_syn':s2_syn_mutation_list, 'rdrp_syn': rdrp_syn_mutation_list, 'spike_syn': spike_syn_mutation_list, 'e_syn': e_syn_mutation_list, 'n_syn':n_syn_mutation_list, 'm_syn': m_syn_mutation_list, 'nsp6_syn': nsp6_syn_mutation_list, 'orf7a_syn': orf7a_syn_mutation_list, 'orf9b_syn': orf9b_syn_mutation_list,'orf8_syn': orf8_syn_mutation_list,'orf7b_syn': orf7b_syn_mutation_list,'orf6_syn': orf6_syn_mutation_list,'orf3a_syn': orf3a_syn_mutation_list,'orf1a_syn': orf1a_syn_mutation_list,'orf1b_syn': orf1b_syn_mutation_list,'rbd_syn': rbd_syn_mutation_list}
    return tree
